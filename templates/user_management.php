...
<SPAN STYLE="font-size:12px; font-weight:bold;">New User <IMG <?=makeButton('create','src')?> STYLE="cursor:pointer;" onClick="location.href='?dispatch=show_admin_add_user'"></SPAN><BR><BR>
<TABLE BORDER=0 WIDTH="100%" CELLSPACING=0>
  <TR CLASS="topic">
<? foreach (array('&nbsp;','Username','Firstname','Lastname','E-Mail','Rights','blocked','Last Login','Action') as $t) : ?>
...
<? $css->switchClass(); ?>
  <TR CLASS="<?=$css->getClass()?>" <?=$css->getHover()?>>
    ...
    <TD CLASS="<?=$css->getClass()?>" VALIGN="top"><IMG <?=makeButton('edit','src')?> onClick="location.href='?dispatch=edit_user&user_id=<?=$u->getUserId()?>'"></TD>
  </TR>
<? endforeach ?>
</TABLE>
